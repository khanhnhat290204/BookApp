...
class Category(db.Model):
    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(String(50), nullable=False)
    book = relationship('Book', secondary='category_book', backref='Category', lazy=True, viewonly=True)
    def __str__(self):
        return self.name
...
if __name__=='__main__':
    with app.app_context():
        ...
        u4=User(username='Storage',password=str(hashlib.md5('storage'.encode('utf-8')).hexdigest()),user_role=UserRole.STORAGE)
        ...
        p1 = Publisher(name="Nhi Dong")

        db.session.add_all([u,u1,u2,u3,u4])
